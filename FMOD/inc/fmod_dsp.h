...
typedef FMOD_RESULT (F_CALLBACK *FMOD_DSP_SHOULDIPROCESS)          (FMOD_DSP_STATE *dsp_state, bool inputsidle, unsigned int length, FMOD_CHANNELMASK inmask, int inchannels, FMOD_SPEAKERMODE speakermode);
...
*/
typedef struct FMOD_DSP_PARAMETER_DESC_INT : public FMOD_DSP_PARAMETER_DESC
{
    ...
    char**                    valuenames;               /* [w] Names for each value.  There should be as many strings as there are posible values (max - min + 1).  Optional. */
} FMOD_DSP_PARAMETER_DESC_INT;
...
*/
typedef struct FMOD_DSP_PARAMETER_DESC_BOOL : public FMOD_DSP_PARAMETER_DESC
{
    bool                      defaultval;               /* [w] Default parameter value. */
    char**                    valuenames;               /* [w] Names for false and true, respectively.  There should be two strings.  Optional. */
} FMOD_DSP_PARAMETER_DESC_BOOL;
...
*/
typedef struct FMOD_DSP_PARAMETER_DESC_DATA : public FMOD_DSP_PARAMETER_DESC
{
...
#define FMOD_DSP_INIT_PARAMDESC_FLOAT(_paramstruct, _name, _label, _description, _min, _max, _defaultval) \
    memset(&_paramstruct, 0, sizeof(FMOD_DSP_PARAMETER_DESC_FLOAT)); \
    ...
    _paramstruct.min          = _min; \
    _paramstruct.max          = _max; \
    _paramstruct.defaultval   = _defaultval; \
    _paramstruct.mapping      = 0;

#define FMOD_DSP_INIT_PARAMDESC_INT(_paramstruct, _name, _label, _description, _min, _max, _defaultval, _goestoinf, _valuenames) \
    memset(&_paramstruct, 0, sizeof(FMOD_DSP_PARAMETER_DESC_INT)); \
    ...
    _paramstruct.max          = _max; \
    _paramstruct.defaultval   = _defaultval; \
    _paramstruct.goestoinf    = _goestoinf; \
    _paramstruct.valuenames   = _valuenames;

#define FMOD_DSP_INIT_PARAMDESC_BOOL(_paramstruct, _name, _label, _description, _defaultval, _valuenames) \
    memset(&_paramstruct, 0, sizeof(FMOD_DSP_PARAMETER_DESC_BOOL)); \
...
#define FMOD_DSP_INIT_PARAMDESC_DATA(_paramstruct, _name, _label, _description, _datatype) \
    memset(&_paramstruct, 0, sizeof(FMOD_DSP_PARAMETER_DESC_DATA)); \
    _paramstruct.type         = FMOD_DSP_PARAMETER_TYPE_DATA; \
    strcpy(_paramstruct.name,  _name); \
    strcpy(_paramstruct.label, _label); \
    _paramstruct.description  = _description; \
    _paramstruct.datatype     = _datatype;


#define FMOD_PLUGIN_SDK_VERSION 102
...
[
    [DESCRIPTION]
    Struct containing callbacks for plugins.
...
struct FMOD_PLUGIN_CALLBACKS
{
    ...
    FMOD_SYSTEM_GETSAMPLERATECALLBACK   getsamplerate;       /* [r] Callback for getting the system samplerate. */
    FMOD_SYSTEM_GETBLOCKSIZECALLBACK    getblocksize;        /* [r] Callback for getting the system's block size.  DSPs will be requested to process blocks of varying length up to this size.*/
};
...
struct FMOD_DSP_STATE
{
    ...
    FMOD_PLUGIN_CALLBACKS          *callbacks;           /* [r] Struct containing callbacks for plugins. */
};

static FMOD_RESULT FMOD_Plugin_GetSampleRate(FMOD_DSP_STATE* state, int *rate)              { return state->callbacks->getsamplerate(state, rate); }
static FMOD_RESULT FMOD_Plugin_GetBlockSize(FMOD_DSP_STATE* state, unsigned int *blocksize) { return state->callbacks->getblocksize(state, blocksize); }
...
typedef enum
{
    ...
    FMOD_DSP_COMPRESSOR_ATTACK,      /* (Type:float) - Attack time (milliseconds), in the range from 10 through 1000. The default value is 20. */
...
[  
    [DESCRIPTION]   
    Parameter types for the FMOD_DSP_TYPE_FFT.
