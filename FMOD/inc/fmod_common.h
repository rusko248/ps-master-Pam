...
/* ============================================================================================== */
/* FMOD Ex - Common C/C++ header file. Copyright (c), Firelight Technologies Pty, Ltd. 2004-2011. */
/*                                                                                                */
...
#define FMOD_VERSION    0x00010100
...
[
    [DESCRIPTION]   
    Used to distinguish if a FMOD_CHANNELCONTROL union parameter is actually a channel or a channelgroup.
...
typedef FMOD_RESULT (F_CALLBACK *FMOD_SYSTEM_GETSAMPLERATECALLBACK)(FMOD_DSP_STATE* thisdsp, int *rate);
typedef FMOD_RESULT (F_CALLBACK *FMOD_SYSTEM_GETBLOCKSIZECALLBACK)(FMOD_DSP_STATE* thisdsp, unsigned int *blocksize);
...
[
    ...
    ----------------------------------<br>
    <br>
    A standard connection will execute its input DSP if it has not been executed before.<br>
    <br>
    FMOD_DSP_CONNECTION_TYPE_SIDECHAIN<br>
    ----------------------------------<br>
    A sidechain is a connection between 2 units, where the output is not mixed into the audible sound path, but instead mixed into a special sidechain that a DSP effect can privately access.<br>
    An example of use in this case, could be a compressor which analyzes the signal, to control its own effect parameters (ie a compression level or gain).<br>
    Sidechain inputs are automatically mixed together, and only with other sidechain inputs.  The 'standard' and 'send' connection types are not affected.<br>
    <br>
    ...
    <br>
    A sidechain connection will execute its input DSP if it has not been executed before.<br>
    <br>
    FMOD_DSP_CONNECTION_TYPE_SEND<br>
    -----------------------------<br>
    <br>
    A send connection will NOT execute its input DSP if it has not been executed before.  It will only read what exists at the input's buffer at the time of executing the output DSP unit (which can be considered the 'return').<br>
    <br>
...
typedef enum
{
    FMOD_DSPCONNECTION_TYPE_STANDARD,          /* Default connection type. */
    FMOD_DSPCONNECTION_TYPE_SIDECHAIN,         /* No interpolation.  High frequency aliasing hiss will be audible depending on the sample rate of the sound. */
    FMOD_DSPCONNECTION_TYPE_SEND,              /* Linear interpolation (default method).  Fast and good quality, causes very slight lowpass effect on low frequency sounds. */

    FMOD_DSPCONNECTION_TYPE_MAX,               /* Maximum number of resample methods supported. */
...
[
    ...
    <li>To specify a starting subsound to seek to within a multi-sample sounds (ie FSB/DLS) when created as a stream.
    <li>To specify which subsounds to load for multi-sample sounds (ie FSB/DLS) so that memory is saved and only a subset is actually loaded/read from disk.
    ...
    <li>To specify a MIDI DLS sample set file to load when opening a MIDI file.
    ...
    <br>
    <br>
    <u>To specify a starting subsound to seek to and flush with, within a multi-sample stream (ie FSB/DLS).</u><br>
    <br>
    <li>Mandatory.  Specify 'initialsubsound'.
    <br>
    <br>
    <u>To specify which subsounds to load for multi-sample sounds (ie FSB/DLS) so that memory is saved and only a subset is actually loaded/read from disk.</u><br>
    <br>
    ...
    <br>
    <br>
    <u>To specify a MIDI DLS sample set file to load when opening a MIDI file.</u><br>
    <br>
